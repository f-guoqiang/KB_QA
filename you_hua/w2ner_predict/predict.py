...
class Trainer(object):
    ...
    def predict(self, predict_loader,res):
      self.model.eval()
      predict_result = []
      with torch.no_grad():
        for i, data_batch in enumerate(predict_loader):
          ...
          # print(outputs)
          predict_decode(outputs.cpu().numpy(), sent_length.cpu().numpy(), texts,res)
...
def predict_decode(outputs, length, texts,res):
  entities = []
  ...
  for x in entities:
    ...
    for i, text in enumerate(x):
        ...
        if i == 0:
            data['text'] = text
            continue
        entities_dict["start_idx"] = text[2]
        entities_dict["end_idx"] = text[3]
        entities_dict["entity"] = text[0]
        entities_dict["type"] = text[1]

        entities_list.append(entities_dict)
        #sorted(entities_list,key=lambda x:x[0])
    data['entities'] = sorted(entities_list,key = lambda x:x['start_idx'])
    # print(data)
    res.append(data)
  # print(res)
  #print(entities)
...
if __name__ == '__main__':
    ...
    import pickle
    with open(r'F:\GitHub\W2NER_predict\CMeEE\test.pkl', 'rb') as f:
        texts = pickle.load(f)
    # texts = ["（5）房室结消融和起搏器植入作为反复发作或难治性心房内折返性心动过速的替代疗法。"]
    ...
    # 加载训练好的模型
    path = r'F:\GitHub\W2NER\mc-bert'
    trainer.load(path+r"\bio_model_2.pt")
    # trainer.eval("Final", test_loader, True)

    # trainer.predict(predict_loader)
    res = []
    ans = trainer.predict(predict_loader,res)
    print(res)
    with open('res_9.json', 'w', encoding='utf-8') as f:
